...
export async function GET(
  ...
  try {
    ...
    const { data: income_items, error } = await supabase
      .from('income_items')
      .select('*')
    ...
    return NextResponse.json(income_items);
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    const body: CreateIncomeItemInput = await request.json();
    ...
    return NextResponse.json(income_item, { status: 201 });
  } catch (error) {
